...
public class DeleteAssetService {
    ...
    public void deleteAsset(Long assetId, Long userId) {
        ...
        if (doc.getStatus().equals("upcoming") || doc.getStatus().equals("active") || doc.getStatus().equals("completed")) {
            throw new AccessDeniedException("Tài sản đã được duyệt, không thể xoá");
        }

        // Xoá image_relation
        imageRelationRepo.deleteByImageFkIdAndType(assetId, ImageRelationType.valueOf("asset"));

        // Nếu tài sản có session chưa được duyệt thì xoá session
        AuctionSession session = doc.getSession();
        if (session != null && session.getStatus().equals("draft")) {
